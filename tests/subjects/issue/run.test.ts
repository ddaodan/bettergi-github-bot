...
describe("runIssueWorkflow", () => {
  it("does not route labeled events into the automatic workflow", () => {
    expect(resolveIssueWorkflowTrigger("labeled")).toBeUndefined();
  });
  ...
  it("runs bilingual AI reply after labeling adds trigger label in the same run", async () => {
    ...
    expect(gateway.comments[0]?.body).not.toContain("Configuration loading fails during startup.");
    expect(gateway.comments[0]?.body).not.toContain("启动时配置加载失败。");
    expect(gateway.comments[0]?.body).not.toContain("### 问题概述");
    expect(gateway.comments[0]?.body).not.toContain("### Summary");
  ...
  it("loads GitHub-hosted log attachments before generating AI help", async () => {
    const config = createConfig();
    config.issues.aiHelp.enabled = true;
    const attachmentUrl = "https://github.com/user-attachments/files/12345678/application.log";
    const issue = createIssue({
      body: [
        "<!-- issue-template: bug -->",
        "",
        ...
        "",
        "## Steps to Reproduce",
        "Start the plugin and wait for the crash",
        "",
        "## Expected Behavior",
        "The plugin should keep running",
        "",
        `## Logs\n[application.log](${attachmentUrl})`
      ].join("\n")
    });
    const gateway = new FakeGateway(issue);
    gateway.textAttachments.set(attachmentUrl, {
      url: attachmentUrl,
      filename: "application.log",
      content: "System.InvalidOperationException: configuration failed",
      truncated: false
    });
    let capturedLog = "";
    const provider = {
      async generateHelp(_issue: unknown, parsed: { textAttachments: Array<{ content: string }> }) {
        capturedLog = parsed.textAttachments[0]?.content ?? "";
        return {
          summary: "配置加载失败。",
          possibleCauses: [],
    ...
    expect(capturedLog).toContain("InvalidOperationException");
  });
